...
//  ConvertIt
//
//  Created by Alex Karacaoglu on 1/28/19.
...
class ViewController: UIViewController {
    ...
    let formulasArray = [Formula(conversionString: "miles to kilometers", formula: {$0 / 0.62137}),
                         ...
                         Formula(conversionString: "inches to cm", formula: {$0 / 0.03937}),
                         Formula(conversionString: "cm to inches", formula: {$0 * 0.03937}),
    ...
    func calculateConversion() {
        guard let inputValue = Double (userInput.text!) else {
            if userInput.text != "" {
                alert(title: "Cannot Convert Value",
                      message: "\"\(userInput.text!)\" is not a valid number.")
            }
            return
        }
